...
public class LaunchScreeenController implements Initializable {
    ...
    @FXML
    private JFXButton load_image_for_copy_move ;
    ...
    private void animate() {
        ...
        TranslateTransition tt1 = new TranslateTransition(Duration.millis(duration), load_image_for_copy_move);
        ...
        ScaleTransition st1 = new ScaleTransition(Duration.millis(duration), load_image_for_copy_move);
        st1.setToX(2);
        st1.setToY(2);
        ...
        tLogo.setToY(50);
        tDesc.setToY(500);
        buttonParallelTransition = new ParallelTransition(load_image_for_copy_move, st1, tt1, tLogo, tDesc);

        buttonParallelTransition.play();
        buttonParallelTransition.setOnFinished((e) -> {
        	load_image_for_copy_move.setOpacity(1);
        });
    ...
    @FXML
    void loadImageSelectorCopyMove(ActionEvent event) {
        if (processingFile != null) {
            return;
        }
        fileChooser = new FileChooser();
    ...
    private void loadCopyMoveCheck() {
    	startSimpleAnimationCopyMove();
    	System.out.println(processingFile);
    	
    	//Calert.showAlert("No File Selected", "You haven't selected any images", AlertType.ERROR);
    	bulgingTransition.setOnFinished((err) -> {
            TranslateTransition tt = new TranslateTransition(Duration.millis(duration - 500), load_image_for_copy_move);
            ScaleTransition st = new ScaleTransition(Duration.millis(duration - 500), load_image_for_copy_move);
            st.setToX(1);
            st.setToY(1);
            ...
            KeyValue keyValueX = new KeyValue(load_image_for_copy_move.prefWidthProperty(), 300);
            KeyValue keyValueY = new KeyValue(load_image_for_copy_move.prefHeightProperty(), 50);
            KeyFrame keyFrame = new KeyFrame(Duration.millis(duration - 500), keyValueX, keyValueY);
            timeline.getKeyFrames().add(keyFrame);

            ParallelTransition pt = new ParallelTransition(load_image_for_copy_move, st, tt, timeline);
            pt.play();
            
            pt.setOnFinished((err1) -> {
            load_image_for_copy_move.setText("       --          ");
            load_image_for_copy_move.setFont(Font.font("Roboto", FontWeight.BOLD, 20));
            homeIcon.setVisible(true);
            try {
                AnchorPane result = FXMLLoader.load(getClass().getResource("/resources/fxml/copymoveresult.fxml"));
...
//                Neural Network Entry
    ...
    private void startSimpleMetaDataAnimation() {
        ...
        load_image_for_copy_move.setVisible(false);
    ...
    private void startSimpleAnimationCopyMove() {
        float animationExtension = 3.25f;
        load_image_button.setVisible(false);
        bulgingTransition = new ScaleTransition(Duration.millis(1000), load_image_for_copy_move);
        bulgingTransition.setToX(animationExtension);
        bulgingTransition.setToY(animationExtension);
        bulgingTransition.autoReverseProperty().setValue(true);
        bulgingTransition.setCycleCount(2);
        bulgingTransition.play();
        load_image_for_copy_move.setFont(Font.font("Roboto", FontWeight.NORMAL, 8));
        load_image_for_copy_move.setText("Detecting Copy Move...");
